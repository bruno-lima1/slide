...
export class Slide {
  ...
  onMove(event) {
    ...
    this.values.movement = this.values.endPosition - this.values.movePosition
    this.moveSlide(this.values.movement)
  }
  moveSlide(movement) {
  ...
  onEnd(event) {
    ...
    this.values.endPosition = this.values.movement
    this.changeSlide()
    this.transition(true)
    this.animation()
    window.dispatchEvent(this.navEvent)
  }
  centralizeSlide(active) {
    this.slideArray = [...this.slide.children].map((element) => {
      const margin = (this.wrapper.offsetWidth - element.offsetWidth) / 2
      const offsetLeft = margin - element.offsetLeft
      return {
        offsetLeft,
        element
      }
    })
    this.moveSlide(this.slideArray[active].offsetLeft)
    this.values.endPosition = this.slideArray[active].offsetLeft
    this.slideIndex(active)
  }
  slideIndex(active) {
    this.index = {
      prev: active - 1 === -1 ? undefined : active - 1,
      active: active,
      next: active + 1 === 6 ? undefined : active + 1
    }
  }
  changeSlide() {
    ...
    } else if (this.index.prev === undefined || this.index.next === undefined) {
...
export default class SlideNav extends Slide {
  constructor (wrapper, slide, nav) {
    super(wrapper, slide)
    this.nav = document.querySelector(nav)
    this.updateNavItem = this.updateNavItem.bind(this)
  }
  slideNav() {
    this.navArray = [...this.nav.children].map((item, index) => {
      item.addEventListener("click", () => {
        this.changeItemNav(item, index)
      })
      return {
        item
      }
    })
    this.navArray[this.index.active].item.classList.add(this.active)
    window.addEventListener("navEvent", this.updateNavItem)
  }
  changeItemNav(item, index) {
    this.navArray = [...this.nav.children].map((item) => {
      item.classList.remove(this.active)
    })
